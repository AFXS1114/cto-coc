...
export default function LeavePrintForm({ leaveId }: { leaveId: string }) {
  ...
  const leaveDocRef = useMemoFirebase(() => {
    ...
    // We don't know the status, so we have to check multiple collections.
    // In a real app, you might pass the status or have a single collection.
    // For now, we'll try `processed-cto` first, then `to-process-leave`.
    return doc(firestore, 'processed-cto', leaveId);
  }, [firestore, leaveId]);

  const { data: processedLeaveData, isLoading: isLoadingProcessed, error: processedError } = useDoc<LeaveData>(leaveDocRef);
  ...
  const cancelledLeaveDocRef = useMemoFirebase(() => {
    if (!firestore || !leaveId || processedLeaveData || pendingLeaveData || isLoadingProcessed || isLoadingPending) return null;
    return doc(firestore, 'cancelled-cto', leaveId);
  }, [firestore, leaveId, processedLeaveData, pendingLeaveData, isLoadingProcessed, isLoadingPending]);

  const { data: cancelledLeaveData, isLoading: isLoadingCancelled } = useDoc<LeaveData>(cancelledLeaveDocRef);


  const leaveData = processedLeaveData || pendingLeaveData || cancelledLeaveData;
